"""편대/군집 비행 공격 테스트 — S101~S108. 결정론·무의존."""
...
def test_eight_swarm_scenarios():
    assert set(SWARM_SCENARIOS) == {f"S{n}" for n in range(101, 109)}
...
def test_leader_spoof_captures_followers():
    r = run_swarm("S101", n=9)
    assert r.swarm_failure and r.detail["captured"] == 8


def test_consensus_poison_exceeds_bft():
    # n=9 → BFT 허용 f=2. 악성 3 > 2 → 합의 붕괴.
    assert run_swarm("S102", n=9, malicious=3).swarm_failure is True
    assert run_swarm("S102", n=9, malicious=2).swarm_failure is False   # 허용 이내


def test_mesh_partition_fragments_swarm():
    assert run_swarm("S108", n=9, malicious=3).swarm_failure is True     # 3 >= 9//3
...
def test_swarm_campaigns_include_swarm_scenarios():
    ...
    for cid in ("C24", "C25", "C26", "C27", "C28", "C29"):
        assert any(s in {f"S{n}" for n in range(101, 109)} for s in CHAINS[cid])
