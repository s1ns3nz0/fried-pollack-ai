...
def test_c14_archive_supply_chain_detected_downstream():
    r = run_chain("C14")                     # S53(사각)→S4→S1
    assert r.verdict == "detected"
    assert ("S53", True, None) in r.stages   # 아카이브 전달은 사각
    assert "S4" in r.detected_at             # 펌웨어 변조에서 탐지


def test_c15_it_to_ot_bridge_caught_at_mission_layer():
    r = run_chain("C15")                     # S49→S51(사각)→S12→S11
    assert r.verdict == "detected"
    assert "S49" not in r.detected_at and "S51" not in r.detected_at   # IT 계층 사각
    assert "S12" in r.detected_at                                      # OT 임무층에서 탐지


def test_c16_pure_it_privesc_fully_stealthy():
    r = run_chain("C16")                     # S49→S50→S51→S52 전부 사각
    assert r.verdict == "stealthy" and r.detected_at == []


def test_c17_supply_web_exfil_caught_at_exfil():
    r = run_chain("C17")                     # S53→S49(사각)→S17
    assert r.verdict == "detected" and r.detected_at == ["S17"]


def test_c18_idor_to_arming_robust_block():
    r = run_chain("C18")                     # S48(사각)→S11(범주형)
    assert r.verdict == "detected" and "S11" in r.detected_at   # 인증우회로도 무장 탐지
