"""RC 조종링크 + DShot 모터 공격 — S29~S8 (Awesome-Drone-Hacking).
...
RF_SCENARIOS = {
    "S29": {"name": "RC 링크 바인딩 탈취", "objective": "rc_link_hijack",
            "mitre": "T1557 / RC bind", "proto": "DSMX/FrSky/ELRS"},
    "S30": {"name": "RC override 명령 주입", "objective": "rc_override",
            "mitre": "T0831 / RC override", "proto": "RC channels"},
    "S31": {"name": "RC 프로토콜 다운그레이드", "objective": "rc_downgrade",
            "mitre": "T1600 / downgrade", "proto": "무암호 폴백"},
    "S8": {"name": "DShot/ESC 모터 조작", "objective": "dshot_motor",
...
def run_rf(scenario_id: str, dry: bool = True) -> RfResult:
    ...
    iface = os.environ.get("SDR_IFACE" if scenario_id != "S8" else "ESC_IFACE", "")
    sent = False
