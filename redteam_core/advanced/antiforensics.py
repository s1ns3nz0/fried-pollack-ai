"""드론 anti-forensics — S40 (Awesome-Drone-Hacking).
...
AF_SCENARIOS = {
    "S40": {"name": "드론 anti-forensics(로그 삭제·블랙박스 조작)",
...
_METHODS = [
    ...
    ("param_reset", "변조 파라미터 원복(S4/S96 흔적 은폐)"),
...
def run_antiforensics(dry: bool = True) -> AfResult:
    ...
    if not dry and os.environ.get("TARGET_HOST"):
        ...
        executed = "sent" in r
    return AfResult("S40", [m for m, _ in _METHODS], executed,
