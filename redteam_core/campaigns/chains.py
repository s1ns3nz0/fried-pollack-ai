...
CHAINS = {
    ...
    "C24": ["S64", "S103", "S109", "S106"],  # 리더참수: 수동정찰→리더 스푸핑→명령 1:N 증폭→편대 분산
    "C25": ["S107", "S104", "S28", "S15"],   # 합의전복: Sybil→합의 포이즈닝→위조 영상/상황도→C4I 위조명령
    "C26": ["S31", "S110", "S1", "S105"],    # 분단격파: C2재밍→메시 파티션→고립멤버 GNSS 스푸핑→충돌
    "C27": ["S76", "S4", "S103", "S109"],    # 공급망전파: CI/CD 침해→펌웨어 백도어→리더 장악→전군집 전파
    "C28": ["S108", "S105", "S110"],         # 물리파괴: flocking 변조→충돌 유도→메시 분단(회복 차단)
    "C29": ["S103", "S109", "S35"],          # 대량유출: 리더 장악→명령 1:N→전군집 SAR/영상 동시 유출
    # ── 운용방식별 캠페인 C30~C33 ──
    "C30": ["S64", "S119", "S113", "S17"],   # 임무탈취: 정찰→GUIDED 강제→추적락 탈취→SAR 유출
    "C31": ["S2", "S115", "S116", "S123"],   # 제어강탈: C2 하이재킹→제어권 전환→자세오염→고정익 실속
